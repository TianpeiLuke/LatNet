...
#===========================================================================
#my own edition
# change alpha to be vector 
@cython.boundscheck(False)
@cython.wraparound(False)
@cython.cdivision(True)
def enet_coordinate_descent_adaptive(np.ndarray[floating, ndim=1] w,
                            np.ndarray[floating, ndim=1] alpha, floating beta,
    ...
    cdef np.ndarray[floating, ndim=1] w_alpha= np.empty(n_features, dtype=dtype)
    #=============================================================
    ...
    # add here
    cdef floating *w_alpha_data = <floating*> w_alpha.data
    #=============================================
    cdef floating *R_data = <floating*> R.data
    cdef floating *XtA_data = <floating*> XtA.data
    
    # add here
    alpha_max = max(n_features, &alpha[0])
    #
    if alpha_max == 0:
        ...
        for n_iter in range(max_iter):
            ...
            for f_iter in range(n_features):  # Loop over coordinates
                ...
                if positive and tmp < 0:
                    ...
                    w[ii] = (fsign(tmp) * fmax(fabs(tmp) - alpha_ii, 0)
                             / (norm_cols_X[ii] + beta))
            ...
            if (w_max == 0.0 or
                ...
                if (dual_norm_XtA > alpha_max):
                    #const = alpha / dual_norm_XtA
                    const = alpha_max / dual_norm_XtA
                ...
                l1_norm = asum(n_features, w_alpha_data, 1)
...
@cython.wraparound(False)
@cython.cdivision(True)
def sparse_enet_coordinate_descent_adaptive(floating [:] w,
    ...
    cdef floating alpha_ii
    cdef floating alpha_max
    cdef floating[:] w_alpha= np.zeros(n_features, dtype=dtype)
    #========================
    cdef floating d_w_max
    cdef floating w_max
    cdef floating d_w_ii
        ...
        for n_iter in range(max_iter):
            ...
            if w_max == 0.0 or d_w_max / w_max < d_w_tol or n_iter == max_iter - 1:
               ...
               # l1_norm = asum(n_features, &w[0], 1)
                l1_norm = asum(n_features, &w_alpha[0], 1)

                gap += (l1_norm - const * dot(
                            n_samples,
                            &R[0], 1,
                            &y[0], n_tasks
                            )
...
@cython.wraparound(False)
@cython.cdivision(True)
def enet_coordinate_descent_gram_adaptive(floating[:] w, floating[:] alpha, floating beta,
    ...
    """Cython version of the coordinate descent algorithm
        for Elastic-Net regression
        We minimize
        (1/2) * w^T Q w - q^T w + norm(w*alpha, 1) + (beta/2) * norm(w, 2)^2
    ...
    with nogil:
        for n_iter in range(max_iter):
            ...
            if w_max == 0.0 or d_w_max / w_max < d_w_tol or n_iter == max_iter - 1:
                ...
                # w_norm2 = np.dot(w, w)
                w_norm2 = dot(n_features, &w[0], 1, &w[0], 1)
                #w_norm2 = dot(n_features, &w_alpha[0], 1, &w_alpha[0], 1)
                ...
                #        0.5 * beta * (1 + const ** 2) * w_norm2)
                gap += (asum(n_features, &w_alpha[0], 1) -
                        const * y_norm2 +  const * q_dot_w +
                        0.5 * beta * (1 + const ** 2) * w_norm2)
...
#@cython.wraparound(False)
#@cython.cdivision(True)
#def enet_coordinate_descent_multi_task_adaptive(double[::1, :] W, double[:] l1_reg,
#                                       double l2_reg,
#                                       np.ndarray[double, ndim=2, mode='fortran'] X,
#                                       np.ndarray[double, ndim=2] Y,
#                                       int max_iter, double tol, object rng,
...
#    cdef double[:] w_ii = np.zeros(n_tasks, dtype=np.float)
#    #========= add here =======================================
#    cdef double alpha_ii
#    cdef double[::1, :] W_alpha= np.zeros((n_tasks, n_features))
#    #==========================================================
#    cdef double d_w_max
#    cdef double w_max
#    cdef double d_w_ii
...
#    cdef double* W_ptr = &W[0, 0]
#    #
#    cdef double* W_alpha_ptr = &W_alpha[0,0]
#    #
#    cdef double* Y_ptr = &Y[0, 0]
#    cdef double* wii_ptr = &w_ii[0]
#
...
#                dcopy(n_tasks, &tmp[0], 1, W_ptr + ii * n_tasks, 1)
#                #============  change here ==========================
#                dscal(n_tasks, fmax(1. - l1_reg[ii] / nn, 0) / (norm_cols_X[ii] + l2_reg),
#                          W_ptr + ii * n_tasks, 1)
#
#                # if np.sum(W[:, ii] ** 2) != 0.0:  # can do better
#                if dnrm2(n_tasks, W_ptr + ii * n_tasks, 1) != 0.0:
...
#                for ii in range(n_features):
#                    for jj in range(n_tasks):
#                        W_alpha[jj, ii] = W[jj, ii] * alpha[ii]
#                #======================================================
#
#                # XtA = np.dot(X.T, R) - l2_reg * W.T
#                for ii in range(n_features):
...
#                R_norm = dnrm2(n_samples * n_tasks, &R[0, 0], 1)
#                w_norm = dnrm2(n_features * n_tasks, W_ptr, 1)
#                if (dual_norm_XtA > array_max(n_features, l1_reg)):
#                    #const =  l1_reg / dual_norm_XtA
#                    const =  array_max(n_features, l1_reg) / dual_norm_XtA
#                    A_norm = R_norm * const
#                    gap = 0.5 * (R_norm ** 2 + A_norm ** 2)
#                else:
#                    const = 1.0
...
#                for ii in range(n_features):
#                    # np.sqrt(np.sum(W ** 2, axis=0))
#                    l21_norm += dnrm2(n_tasks, W_alpha_ptr + n_tasks * ii, 1)
#
#                #gap += l1_reg * l21_norm - const * ry_sum + \
#                #     0.5 * l2_reg * (1 + const ** 2) * (w_norm ** 2)
#                gap += l21_norm - const * ry_sum + \
#                     0.5 * l2_reg * (1 + const ** 2) * (w_norm ** 2)
#
#                if gap < tol:
#                    # return if we reached desired tolerance
